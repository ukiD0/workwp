...
class WP_Ivecountdown {
	...
	function ive_shortcode($atts, $content=null) {
	  ...
	  // deal with display years
		...
		$ive = '<div id="'.$id.'-countdown" class="ive-title countdown_main_'.$id.' align'.$blockalignment.' ive_countdown" style="width:'. esc_attr($width) .'; height:'. esc_attr($height) .';">';
				...
				$('#".$id."-dashboard').iveCountDown({
					...
					if(!empty($content)){
						$ive .= ", onComplete: function() {
							$('#".$id."-".$launchtarget."').css({'width' : '". esc_js($launchwidth)."', 'height' : '".esc_js($launchheight)."'});

...
function post_table_shortcode($atts)
{
	...
	if ((isset($atts)) && (is_array($atts)) && (count($atts) > 0) && ((isset($atts['catid']))))
	{
	    if (isset($atts['catid']))
	    {
	    ...
	    if (isset($atts['catname']))
	    {
	        ...
	        if ((is_array($user_args_catid_array)) && (count($user_args_catid_array) > 0))
	        {
	            $user_args_catid_query = implode("','", $user_args_catid_array);
					...
					AND post_status='publish'
					AND $wpdb->term_taxonomy.taxonomy = 'category'
					AND $wpdb->terms.name IN('$user_args_catid_query')
					ORDER BY wposts.post_title ASC $limit_sql
					";
	        }
	    }
   ...
   //before 8.3.5 $sql = $wpdb->prepare( "SELECT ID, post_title, post_content, post_excerpt FROM $wpdb->posts WHERE post_type=%s AND post_status='publish' order by post_title ASC $limit_sql",$post_type);
